...
class Instrumenting_Final_Verson_Apps:
	...
	def Filter_DF_Info_By_Folder_Path(self):
		this_list = os.listdir(self.testing_folder_path)
		this_list = [w.replace('signed', '') for w in this_list]
		# print(this_list)

		self.df_apps_filtered = self.df_apps_filtered[self.df_apps_filtered['App_Name'].isin(this_list)]
	...
	def Instrument_Apps_In_Testing_Folder_Path(self):
		...
		for file in os.listdir():
			...
			main_activity=df_filtered['Main_Activity']
			main_class=df_filtered['Main_Class']
	...
	def Instrument_Apps_In_Testing_Folder_Path_Install_And_Start(self):
			...
			for file in os.listdir():
				...
				cmd= ' '.join(['aapt dump badging',file])
				# result = os.system(cmd)
				direct_output = subprocess.check_output(cmd, shell=True)
				result = str(direct_output).split('\n')

				print(result)
				# print('launchable-activity' in str(result))
				# break

				if self.generic_file_name in df_all_apps['App_Name'].values and 'launchable-activity' in str(result):
					cmd= ' '.join(['adb install-multiple', file])
					os.system(cmd)
					# cprint(file, 'green')
					df_filtered = df_all_apps[df_all_apps['App_Name'] == generic_file_name]
					main_activity = df_filtered.Main_Activity.iloc[0]
					main_class = df_filtered.Main_Class.iloc[0]
					...
					cprint(''.join(['\tRunning app: ', str(app_counter),' of ',str(number_of_apps)]), 'green')
					print('\nApp_Name:', generic_file_name)
					# print('\n File name for df is:',df_filtered['App_Name'])
					print('Package name is:', self.package_name)
					print('Main Activity name is:', self.main_activity)
	...
	def Start_Logcat(self):
		cprint("Logging to: "+self.logcat_path, 'yellow')
...
# instrument_apps.Filter_DF_Info_By_Folder_Path()

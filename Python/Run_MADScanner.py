...
def Copy_Files_To_Appropriate_Folders():
	...
	for this_dir in dir_list:
		dst_dir=''.join([dir_to_move_files_to,'/',this_dir])
		print("this_dir!!!:", this_dir)
		result = [v for v in os.listdir(this_dir) if v.endswith('jimple')]
		if(len(result)>0):
			helper.Copy_Files_In_Directory_To_Other_Directory(this_dir, dst_dir)
			# print(this_dir)
			...
			# shutil.copytree(this_dir, dir_to_move_files_to)
			# helper.Copy_Files_In_Directory_To_Other_Directory(this_dir, dst_dir)
	...
	for this_apk in os.listdir():
		...
		if this_apk.endswith('.apk'):
			try:
				...
				cprint(''.join(['Error With Zipping and Signing APK: ', this_apk]), 'red')
	os.chdir(cwd)
...
def Copy_Signed_Files_In_sootOutput_Folder_To_Destination_Folders():
	cwd=os.getcwd()
	helper=Helper.Helper()
	dir_to_move_files_to='../../../Data/Application_Analysis'
	if os.path.exists('../Java/Classes/sootOutput'):
		os.chdir('../Java/Classes/sootOutput')
		for file in os.listdir():
			if file.startswith('signed'):
				src=file
				dst=''.join([dir_to_move_files_to,'/',file.replace('signed','')])
				print(src, ' ', dst)
				shutil.copy2(src, dst)
				os.remove()
				# helper.Copy_Files_In_Directory_To_Other_Directory(src, dst)
	os.chdir(cwd)

def Clean_Classes_Folder():
	cwd=os.getcwd()
	# helper=Helper.Helper()
	os.chdir('../Java/Classes/')
	# dir_to_move_to=
...
def Instrument_Apps(testing_folder_path, this_test_folder_path):
	pwd = os.getcwd()
	# print(pwd)
	df = pd.read_csv(''.join(['../Data/',this_test_folder_path,'.csv']), index_col=False)
	...
	if len(this_phone_name) > 0:
		...
		if df.size > 0:
			...
			for idx,row in df.iterrows():
				...
				this_main_class=row['Main_Class']
				this_main_activity=row['Main_Activity']
				# cprint(this_app_name,'yellow')
				this_file_signed=''.join(['signed',this_app_name])
				this_file_path=''.join([testing_folder_path,'/',this_app_name, '/',this_file_signed])
				# cprint(this_file_path,'yellow')
				if os.path.exists(this_file_path):
					...
					cprint(''.join(['no file path for: ', this_file_path]), 'red')
					cprint(os.getcwd(), 'yellow')
					os.chdir(pwd)
			os.chdir('../Data/')
...
Clean_Classes_Folder()
Copy_Signed_Files_In_sootOutput_Folder_To_Destination_Folders()
